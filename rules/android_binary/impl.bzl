...
def _process_resources(ctx, manifest_ctx, java_package, **unused_ctxs):
    ...
    packaged_resources_ctx = _resources.package(
        ...
        has_local_proguard_specs = ctx.attr.proguard_specs,
...
def _process_art_profile(ctx, bp_ctx, dex_ctx, optimize_ctx, **_unused_ctxs):
    ...
    if (ctx.attr.generate_art_profile and
        ...
        if merged_baseline_profile:
            providers.extend(
                _baseline_profiles.process_art_profile(
                    ctx,
...
def _process_r8(ctx, jvm_ctx, packaged_resources_ctx, build_info_ctx, **_unused_ctxs):
    ...
    # Propagate transitive proguard specs unconditionally
    # for the case this binary is a dependency of android_application
    proguard_specs = proguard.get_proguard_specs(
        ctx,
        packaged_resources_ctx.resource_proguard_config,
        check_local_specs = False,
    )
    ...
    if not acls.use_r8(str(ctx.label)) or not local_proguard_specs:
        return ProviderInfo(
            ...
            value = struct(
                ...
                providers = [ProguardSpecProvider(depset(proguard_specs))]
            ),
        )
    ...
    if ctx.attr.baseline_profile:
        rewritten_baseline_profile = ctx.actions.declare_file(ctx.label.name + "_baseline_profile_rewritten.txt")

    resources_class_jar = packaged_resources_ctx.class_jar
    android_dex_info = r8.process(
        ctx,
        deploy_jar,
        proguard_specs,
    ...
    return ProviderInfo(
        name = "r8_ctx",
        value = struct(
            final_classes_dex_zip = android_dex_info.final_classes_dex_zip,
            final_proguard_output_map = android_dex_info.final_proguard_output_map,
            rewritten_baseline_profile = rewritten_baseline_profile,
            providers = [android_dex_info],
        ),
    )

def _process_art_profile_r8(ctx, r8_ctx, **_unused_ctxs):
    providers = []
    if ctx.attr.baseline_profile and r8_ctx.rewritten_baseline_profile:
        providers.extend(
            _baseline_profiles.process_art_profile(
                ctx,
                final_classes_dex = r8_ctx.final_classes_dex_zip,
                merged_profile = r8_ctx.rewritten_baseline_profile,
                profgen = get_android_toolchain(ctx).profgen.files_to_run,
                zipper = get_android_toolchain(ctx).zipper.files_to_run,
                toolchain_type = ANDROID_TOOLCHAIN_TYPE,
            ).providers
...
def _process_resource_shrinking_r8(ctx, r8_ctx, packaged_resources_ctx, **_unused_ctxs):
    ...
    Returns:
      The r8_ctx ProviderInfo.
    """
    resource_info = r8.process_resource_shrinking(
        ctx,
        r8_ctx.final_classes_dex_zip,
        r8_ctx.final_proguard_output_map,
        packaged_resources_ctx.resources_apk,
        packaged_resources_ctx.android_application_resource
    )
    return ProviderInfo(
        name = "resource_shrinking_r8_ctx",
        value = struct(
            android_application_resource_info_with_shrunk_resource_apk = resource_info,
            providers = [],
        ),
    )
...
PROCESSORS = dict(
    ...
    R8Processor = _process_r8,
    ArtProfileProcessorR8 = _process_art_profile_r8,
    ResourecShrinkerR8Processor = _process_resource_shrinking_r8,
)
